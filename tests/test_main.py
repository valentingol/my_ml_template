...
def test_main(mocker: MockerFixture) -> None:
    ...
    # training mode
    run = os.system("python template/main.py --mode='train' "
                    "--experiment_path='tests/tmp/1'")
    check.equal(run, 0)
    check.is_true(os.path.exists("tests/tmp/1_0/config.yaml"))
    check.is_true(os.path.exists("tests/tmp/1_0/config_hierarchy.yaml"))
    # inference mode
    run = os.system("python template/main.py --mode='infer' "
                    "--experiment_path='tests/tmp/2'")
    check.equal(run, 0)
    check.is_true(os.path.exists("tests/tmp/1_0/config.yaml"))
    check.is_true(os.path.exists("tests/tmp/1_0/config_hierarchy.yaml"))
    # unknown mode
    run = os.system("python template/main.py --mode='UNKNOWN' "
                    "--experiment_path='tests/tmp/3'")
